...
Use `trial_success()` to express what “success” means for the trial. The
variables `r1, r2, r3, r4` are binary indicators — `ri = 1` if
hypothesis Hi is rejected, 0 otherwise. You can combine them with
...
Use `graph_constraint()` to encode structural rules — for example, that
only primaries receive initial alpha, and that secondaries can only
receive alpha recycled from their gating primary:
