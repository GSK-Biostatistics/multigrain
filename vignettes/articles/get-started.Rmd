...
you should specify a one-sided alpha level (e.g., $\alpha = 0.025$).
...
If you already have a good starting graph, you can skip the global
search entirely:
...
For large problems with many hypotheses (m ≥ 6), enable parallel execution via
the `num_threads` argument of `graph_optimise()`:
...
For more detail on constraining graph structure, see the
[Graph Constraints](graph_constraint.html) vignette.
