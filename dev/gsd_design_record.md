...
Status: design specification. Phases P0 to P3 are implemented on branch `gsd-build` (P0 to P2 on 2026-09-16, P3 on 2026-09-17; see `dev/gsd_progress.md`); P4 and P5 are not. Corrections made in the light of the implementation and its reviews are marked `[Rev 2026-09-16]`, `[Rev 2026-09-17]` or `[Rev 2026-09-18]`. The last set comes from the P4 planning session: the Sol-5-6 review of `trial_success_gsd()` (`dev/review/trial_success_gsd/ts_gsd_vignette_findings.md`, items 2 to 9), the user's decisions on those items, and the Figure 3b reference recovered from the paper's code archive (`dev/review/fig3b/`). It supersedes `debug/docs/stale-design.md` (April 2026) in full, including the three decisions that document called governing; section 2 says why. Every design element is stated, followed by the reasoning behind it and the alternatives it was chosen over. Elements are marked **LOCKED** when the reasoning is considered settled and **OPEN** when evidence or a decision is still needed; open items are collected in section 10. Claims about `gsDesign`, `graphicalMCP` and the existing package were checked by running code; the scripts and their output are in the appendices.
...
This was checked, not assumed. Inverting `gsDesign` boundaries reproduces the nominal boundaries that Maurer and Bretz published to four significant figures and their repeated p-values (computed with ADDPLAN) to about three and a half: three of the eight Table 2 values differ from the paper's fourth digit by up to $8.5\times10^{-5}$, which is the ADDPLAN versus `gsDesign` residual `[Rev 2026-09-16]`. The existing cascade on those values reproduces their worked example (Appendix A). A direct implementation that computes boundaries on the fly agreed with transform-then-cascade on every one of 1200 hypothesis-by-trial decisions and decision times, in four look-back configurations (Appendix B).
...
**Rationale.** LOCKED. Correctness rests on section 3.3 of Maurer and Bretz: under well-ordered boundaries $p_{i,k}\le\alpha^*_{i,k}(a)\iff p^r_{i,k}\le a$, and $p^r$ does not involve the graph. Appendix A reproduces their Table 1 boundaries from `gsBound1` to four significant figures, their Table 2 repeated p-values to about three and a half (0.168214, 0.131537 and 0.0116485 against the paper's 0.1683, 0.1316 and 0.0117; the gate asserts the Appendix A values `[Rev 2026-09-16]`), and reproduces the rejections in their case study. Appendix B shows the whole chain (transform, then $K$ passes of the fixed-sample cascade) agreeing with a direct implementation on 0 of 1200 decisions and 0 of 1200 decision times in each of four look-back configurations, including one hypothesis maturing at look 2 and one at look 1.
...
Spending function interface: any `function(alpha, t)` returning a numeric vector of cumulative spend or an object with a `$spend` element, evaluated at the full vector $t_i$. `gsDesign` functions work directly (`sfLDOF`, `sfLDPocock`, `sfPower`; parametrised ones as `function(a, t) gsDesign::sfHSD(a, t, param = -2)`), and so do those of `graphicalMCP` (`spending_of()` returns a plain numeric vector). The converse does not hold: `graphicalMCP`'s own `graph_test_shortcut_gsd()` indexes the spending function's return value as a numeric vector, so a `gsDesign` function passed to the oracle silently yields repeated p-values of 1 with only a coercion warning; oracle tests must wrap it as `function(a, t) gsDesign::sfLDOF(a, t)$spend` `[Rev 2026-09-16]`. The transform checks $\phi_i(a,1)=a$ within tolerance ($10^{-6}\alpha$ relative as implemented; a spending function that does not spend everything by full information would silently waste alpha).
...
**Decision.** LOCKED. $\tau_i$ is the look index at which the kernel processed the rejection of $H_i$. Under both semantics this equals the manuscript's definition (the first analysis at which the procedure declares $H_i$ rejected) and `decision_at` in `graphicalMCP`. In Example 5 a PFS rejection that becomes possible only when OS recycles alpha at the final analysis gets $\tau_{\text{PFS}}=2$, as the manuscript specifies. The `first_rejected_at` quantity of `graphicalMCP` (the earliest look whose boundary the hypothesis crossed, under look-back) is a reporting quantity, is not needed by the gain, and can be recovered from the transformed array if a user asks. Note that the oracle's `decision_at` is written for every hypothesis *tested* at an analysis, so a never-rejected hypothesis carries the index of the last analysis at which it was live rather than "never"; it is comparable to `time` only on the rejected set `[Rev 2026-09-16]`.

Calendar times $s_k$ enter only through the user's discount tables (section 4.6). `[Rev 2026-09-18]` They are not stored: no argument of `simulate_pvalues_gsd()`, `transform_pvalues_gsd()` or `trial_success_gsd()` accepts them, and the earlier sentence "the package stores them, if supplied, as labels" described a supply path that was never specified (Sol item 7). Every reported quantity is on the look-index scale: `calc_power_pvals_gsd()` returns `mean_decision_look`, not a calendar mean, and labels the decision-time distribution by analysis number. Averaging look indices and converting afterwards would be wrong for unequally spaced analyses, so no calendar quantity is derived. Optional analysis-time metadata on the `multigrain_pvals_gsd` object is OPEN item 12, deferred by user decision.
...
**Parsing and precedence** `[Rev 2026-09-17]`. The fixed-sample parser rewrites `&&`, `||`, `and` and `or` to the placeholders `%AND%` and `%OR%` before calling `str2lang()`. That cannot be copied: R parses `%op%` above `==`, and comparison operators are non-associative, so `str2lang("t1 == 1 %AND% t2 == 1")` is a parse error (verified on R 4.6.1). `trial_success_gsd()` therefore maps the word forms `and`/`or` (string input only) to `&&`/`||` and lets R parse the native operators; the `&&` and `||` calls are then handled directly in the transformer with the same bool-only rule and the same emitted C++ (`A * B` and `std_min(double(1), A + B)`). The consequence is that the GSD grammar has R's own precedence: `r1 + r2 && r3` is `(r1 + r2) && r3`, which the bool rule rejects with an error asking for parentheses, whereas the fixed-sample parser's placeholders regroup it as `r1 + (r2 && r3)` because `%op%` binds tighter than `+` (this is locked in by `tests/testthat/test-trial_success.R:306-309`). The bool rule itself is unchanged. The fixed-sample behaviour is a pre-existing quirk, recorded in section 9 alongside the two others, and is not changed by this work. All the examples above parenthesise their comparisons and are unaffected. One case is silent rather than an error, and a user porting a `trial_success()` string must know it: `||` and `&&` have equal precedence under the placeholders (both are `%op%`, grouped left to right), so the fixed-sample parser reads `r1 || r2 && r3` as `(r1 || r2) && r3`, while `trial_success_gsd()` reads it as R does, `r1 || (r2 && r3)`. Both compile without warning and give different gains (the P3 review measured 0.571 against 0.857 on one matrix). The test file asserts the R reading.

Two protections added by the implementation `[Rev 2026-09-17]` after the P3 review: the generated function checks once per call that the time matrix has at least `m` columns and errors otherwise (without it a narrower matrix read past the row and crashed R), and injected numeric constants are written with the fewest significant digits that round-trip (15, 16 or 17), so `!!(1/3) * r1` evaluates to exactly `1/3` where the fixed-sample parser's `as.character()` loses the last two digits. What is not checked, by decision, is the value of each time: `name_tab[t(i, idx)]` is an unchecked C array index, so a time above `K` reads garbage and an `NA` time (`INT_MIN` in C) crashes R. Only the kernel of 4.3 may feed the compiled function, and P4 asserts `gain$K == pvals$K` and `!anyNA()` before it does (section 10 item 6).

Two further details fixed by the implementation `[Rev 2026-09-17]`: a discount table name must be a C identifier (letters, digits, underscores, starting with a letter) that is not `r<i>`, `t<i>`, `and` or `or`, since the table becomes `static const double <name>_tab[]` and those names are recognised by pattern before table names; and a numeric literal is written to C++ as a double literal only if it carries no decimal point or exponent already (`1e-05` is kept, `2` becomes `2.0`), where the fixed-sample parser would emit the invalid `1e-05.0`. Logical literals `TRUE`/`FALSE` become `1.0`/`0.0` of type bool. Table values and constants are written with the fewest significant digits (15, 16 or 17) that read back to the same double, so `0.75` stays `0.75` and `1/3` becomes `0.3333333333333333`.
...
- `graph_optimise_gsd(pvals, graph_constraint, trial_success, ..., alpha = NULL, start_graph, global_search, num_threads, control, verbose)`: the same signature and flow as `graph_optimise()`, with `pvals` a `multigrain_pvals_gsd`. `[Rev 2026-09-18]` `trial_success` must be a `multigrain_trial_success_gsd` (a fixed-sample gain is refused with a pointer to `trial_success_gsd()`, whose `r1 + r2` form covers rejection-only gains); before the first evaluation the function asserts `trial_success$m == pvals$m` and, when `trial_success$K` is not `NULL`, `trial_success$K == pvals$K` (section 10 item 6), and `!anyNA()` on the reshaped array (item 11). `alpha` defaults to `pvals$alpha`, the level the boundary tables were built up to; a supplied value must lie in $(0, \texttt{pvals\$alpha}]$, because repeated p-values are capped at 1 above the top of the table (4.1 step 4) and an allocation beyond it could never reject. Internally: `create_obj_func_gsd()` (a copy of the decode-and-penalise block calling the GSD kernel and passing `time` to the gain), `.graph_optimise_ga_gsd()` and `.graph_optimise_local_gsd()` (subsampling on the first array dimension with `drop = FALSE`; re-evaluation with the GSD kernel), `choose_graph()` unchanged, `prune_graph_gsd()` and `calc_power_pvals_gsd()`. Control calibration reads $N$ and $m$ from the object through a new internal `control_prepare_dims()` rather than from a matrix. Returns a `multigrain_graph_optimal`.
- `calc_power_pvals_gsd(pvals, hyp_weight, trans_matrix, ..., alpha = NULL, custom_power, sum_to_one_constraint)`: returns `local_power` (probability of rejecting each hypothesis at any look), `local_power_by_analysis` ($m\times K$, cumulative by look, so its last column equals `local_power`), `exp_rejections`, `disj_power`, `conj_power`, `mean_decision_look` (mean look index over the trials in which the hypothesis was rejected; `NA` when it never is) `[Rev 2026-09-18, was mean_decision_time; see 4.4]`, `time_distribution` ($m\times(K+1)$, proportions at "never" and looks $1..K$, columns labelled `never`, `analysis 1`, ..., rows summing to one), and one entry per `custom_power` measure. `[Rev 2026-09-18]` Dispatch on the gain type is explicit (Sol item 5): a `multigrain_trial_success_gsd` entry is evaluated by its compiled function on the kernel's integer time matrix, after the same `m`, `K` and `anyNA()` assertions as the optimiser; a fixed-sample `multigrain_trial_success` entry is evaluated on the kernel's logical `rejected` matrix (so a rejection-only gain built with `trial_success()` reports the same number here as `calc_power_pvals()` would at $K=1$); a plain R function receives the integer time row. `alpha` follows the same default and bound as `graph_optimise_gsd()`. After the kernel returns, the function asserts `max(time) <= pvals$K` once (cheap on the public path; the objective closure does not repeat it).
...
**Rationale.** The family is additive: no existing exported function, internal helper or kernel changes, so `graph_optimise()` on `main` and on the feature branch are the same code, and the gate in section 6 (identical results for the same seed) is a check on that fact rather than a fixture. The cost is duplicated plumbing (objective closure, two optimiser drivers, pruning acceptance); section 10 records the follow-up to merge the two paths once the sparsity branch has landed. Returning the same result class means `plot()`, `print()`, `summary()` and `graph_optimal_get_control()` work unchanged. `[Rev 2026-09-18]` The last clause of this paragraph ("`summary()` will print the by-look power matrix acceptably") is wrong as implemented: `summarise_power_object()` in `R/graph_optimal.R` prints only the fields it knows (`trial_success`, `local_power`, `exp_rejections`, `disj_power`, `conj_power`), so `local_power_by_analysis`, `mean_decision_look` and `time_distribution` are carried on the object but never shown. Nothing fails; the extra fields are simply invisible in `print()` and `summary()`. Displaying them is a P5 decision, and would mean editing `R/graph_optimal.R`, which P4 does not touch.
...
*`graphicalMCP` oracle conventions* `[Rev 2026-09-16]`, established while writing the P0 and P2 oracle tests and to be respected by every later oracle test: (i) `spending_fn` must return a plain numeric vector (see 4.1); (ii) the `p` matrix must carry no column names, because `gsd_input_val()` compares `identical(is.na(p), is.na(info_frac))` including dimnames and fails with a misleading "NA positions" message otherwise; (iii) `decision_at` is written for tested-but-not-rejected hypotheses (see 4.4), so compare on the rejected set only; (iv) the oracle rejects with `<=`, the kernel with `<`, so keep oracle inputs away from boundaries; (v) `repeated_p()` roots to $10^{-6}$ absolute on the level scale, clamps to $[10^{-6},1-10^{-6}]$, and at $K\ge3$ is not reproducible run to run; (vi) our repeated p-values are capped at 1 above $\alpha$ (4.1 step 4) where the oracle reports the true value, so "oracle agreement" means agreement on values at or below $\alpha$ and on decisions, not elementwise equality of the transformed arrays.
...
No file under `R/` or `src/` that exists today is edited except `R/RcppExports.R` and `src/RcppExports.cpp` (both regenerated by `Rcpp::compileAttributes()` `[Rev 2026-09-16]`), `NAMESPACE` (regenerated by roxygen) and `DESCRIPTION`; new code lives in new files. The gate is an end-to-end run of `graph_optimise()` with a fixed seed on `main` and on the branch, compared with `identical()` on the returned object, plus `identical()` on `.Random.seed` afterwards. `[Rev 2026-09-18]` `identical()` on the *whole* object can never hold when the two runs are in separate R processes, whatever the code: the returned `trial_success` element carries the function compiled by `Rcpp::sourceCpp()`, whose environment holds an external pointer that differs between processes. Measured on the P4 build: `.Random.seed` identical, and `hyp_weight`, `trans_matrix`, `power`, `solution` and `global_output` each `identical()`, with `trial_success` the only top-level component that is not. The gate is met when every component other than `trial_success` is `identical()`.

`[Rev 2026-09-18]` One exception, by user decision (Sol item 5). Because `multigrain_trial_success_gsd` inherits from `multigrain_trial_success`, `check_trial_success()` accepts a GSD gain, and `graph_optimise()` and `calc_power_pvals()` then hand its compiled function the logical rejection matrix; Rcpp coerces `TRUE`/`FALSE` to the integers 1/0, so every rejection is scored as an analysis-1 rejection and a plausible but wrong number is returned without any error (measured: 0.583 on the true decision times against 0.667 on the logical matrix). P4 therefore adds one abort in `graph_optimise()` (after `check_trial_success()`) and one in `.auto_name_custom_power()` in `R/calc_power.R`, both keyed on `is_trial_success_gsd()` and both pointing the user to the `_gsd` consumer. `graph_optimal()`, the neutral result container, keeps accepting both classes. The two edits add no code path for fixed-sample gains, so the identity gate above is unaffected; they are kept in their own commit so that the sparsity branch can rebase past them.
...
Gate: `tests/testthat/test-transform_pvalues_gsd.R` passes; it asserts Table 1 boundaries and Table 2 repeated p-values of Maurer and Bretz to four significant figures against the Appendix A `gsDesign` values (the paper's own Table 2 digits are matched only to about three and a half; see 4.1 `[Rev 2026-09-16]`), that a deliberately non-monotone spending function aborts, that the grid inverse is within $10^{-5}$ of `uniroot` at 200 random p-values, that the first-look short-circuit returns the raw column `identical()`, and agreement with `graphicalMCP::repeated_p()` and `sequential_p()` within $10^{-5}$ **absolute** on random p-values in $[10^{-5}, 10^{-2}]$ at $K=2$ (`skip_if_not_installed`). The oracle roots to `tol = 1e-6` on the level scale and clamps to $[10^{-6}, 1-10^{-6}]$, so relative agreement at $10^{-5}$ is not achievable, and at $K\ge3$ its `pmvnorm` is randomised so it is not reproducible run to run `[Rev 2026-09-16]`. **Done: `e4ec646`.**
...
Gate: `test-sim_pvals_gsd.R`: dimensions and the `info_frac` attribute; empirical within-hypothesis correlation $\sqrt{t_k/t_l}$ and cross-hypothesis correlation $\rho\sqrt{\min/\max}$ within $5\times10^{-3}$ at $N=10^6$ (`skip_on_cran`; set `NOT_CRAN=true` when running from `Rscript` or the block silently skips `[Rev 2026-09-16]`); matured columns identical. **Done: `96dba9f`.**

**P2. Kernel.** (3 days)
Create `src/graph_shortcut_gsd.cpp` with the serial and parallel functions of 4.3; regenerate `R/RcppExports.R` and `src/RcppExports.cpp`. Create `tests/testthat/helper-gsd_reference.R` containing the direct boundary-on-the-fly R implementation from Appendix B (as `gsd_reference_direct()` with its helpers, plus `gsd_pvals_matrix()` for the `dim<-` reshape). As in Appendix B, the reference requires every hypothesis to reach $t\ge1$ and has no `NA` handling in `tmat`; the never-matures and `NA`-padded cases are covered by the oracle and by ad hoc review scripts, not by this helper `[Rev 2026-09-16]`. Oracle tests must observe the `graphicalMCP` conventions listed in 4.9. **Done: `d0b6556`.**
...
`[Rev 2026-09-17]` The "copy with three additions" holds for the validator, index counter and constructor, but not for the placeholder substitution in `replace_r_indices()`, which had to be replaced by native parsing (4.6, "Parsing and precedence"). The implementation also has its own helpers `.gsd_gain_tables()` and `.gsd_gain_K()` for the `...` tables and `K`, and the validator has two extra checks (a table call must take a single bare `t<i>`; a literal must be a scalar, so an injected vector is caught before compilation). `print()` and `summary()` methods exist for the class and add the `K` and the tables to the fixed-sample output. `K` is left `NULL` when neither a table nor the argument fixes it (section 10 item 6 as recorded). The signature is `trial_success_gsd(objective, ..., K = NULL, verbose = multigrain_verbosity())`. **Done: see `dev/gsd_progress.md`.**
`[Rev 2026-09-18]` Amendment made alongside P4 (Sol item 9, user decision): `.gsd_gain_tables()` warns, once per table, when a discount table contains a value below 0 or above 1, or increases from one analysis to a later one (tolerance `sqrt(.Machine$double.eps)`), naming the table and the offending positions or values; when the first value is 0 the warning adds that `trial_success_gsd()` supplies `d(0) = 0` itself, since `c(0, 1, 0.75)` is the shifted-by-one mistake. It is a warning, not an error: the manuscript defines a discount function on $[0,1]$ and non-increasing in time, but the package does not forbid other value tables. The first value is not required to be 1. The roxygen paragraph that called non-monotone tables "by design" is replaced by this policy, and the note that no public consumer exists is removed.

**P4. Optimiser and post-processing.** (3 days)
Create `R/objective_function_gsd.R` (`create_obj_func_gsd()`), `R/optimisation_gsd.R` (`graph_optimise_gsd()`, `.graph_optimise_ga_gsd()`, `.graph_optimise_local_gsd()`, `control_prepare_dims()`, `.sample_pvals_gsd()`), `R/calc_power_gsd.R` (`calc_power_pvals_gsd()`, `.auto_name_custom_power_gsd()`, `.eval_custom_power_gsd()`), `R/post_optim_processing_gsd.R` (`prune_graph_gsd()`, `prune_hyp_weights_gsd()`, `prune_edges_gsd()`, `.try_prune_gsd()`) and `[Rev 2026-09-18]` `R/check_gsd.R` (`is_pvals_gsd()`, `check_pvals_gsd()`, `check_trial_success_gsd()`, `.gsd_check_gain_dims()`, `.gsd_check_alpha()`, `.gsd_kernel_matrix()`, the last being the `dim<-` reshape of 4.1 with the single `anyNA()` abort of section 10 item 11). Reuse unchanged: `split_theta()`, `recover_full_weights()`, `recover_full_trans_matrix()`, `param_to_solution()`, `repair_graph()`, `.build_start_matrix()`, `create_start_params()`, `choose_graph()`, `graph_optimal()`, `.redistribute_mass()`, `.marginal_violated()`. Subsampling for the GA and COBYLA stages uses `sample.int(nsim, size, replace = FALSE)` as the fixed path's `.sample_pvals_rows()` now does (the "first `nsim` rows" issue of section 9 was fixed on `main` in `e62dfcf`), applied to the `multigrain_pvals_gsd` object with `drop = FALSE`.
`[Rev 2026-09-18]` Also in this phase, in separate commits: the fixed-consumer guard of 4.10 (`R/optimisation.R`, `R/calc_power.R`, `tests/testthat/test-gsd_guards.R`, a `NEWS.md` bug-fix line) and the discount-table warning of P3 above.
Gate: `test-optimisation_gsd.R` runs the whole pipeline at $m=3$, $K=2$, $N=2000$ and returns a valid graph with a populated `power` element, and also covers `global_search = FALSE`, a user `start_graph`, the `m`/`K`/gain-class/`alpha` aborts, subsampling keeping a three-dimensional array (section 8 item 12) and `print()`/`summary()` (item 16); `test-objective_function_gsd.R` checks the closure against the kernel and gain called by hand, the `identical()` gain on repeated evaluation (item 11), the `anyNA()` abort, and that the penalty branches return the same values as `create_obj_func()` on the same encoded vector; `test-post_optim_processing_gsd.R` shows pruning never lowers the gain and respects fixed entries; `test-calc_power_gsd.R` checks shapes and dimnames, that `local_power` equals the last column of `local_power_by_analysis`, that rows of `time_distribution` sum to one, `mean_decision_look` against a hand computation (with `NA` for a never-rejected hypothesis), that at $K=1$ the shared fields equal `calc_power_pvals()` on the same matrix, and all three kinds of `custom_power`. The no-change gate of 4.10 is run as a script (`dev/gsd_identity_check.R`) against an installed `main` (`324b7ca`) and an installed branch build in separate libraries and `Rscript` processes, and its output is pasted into the report; if `identical()` fails only on closures carried inside `global_output`, the script reports `all.equal()` on `hyp_weight`, `trans_matrix`, `power`, `solution` and `global_output@solution` together with `identical()` on `.Random.seed`, and says so. `[Rev 2026-09-18]` In practice the whole-object `identical()` is always `FALSE` across processes, for the reason given in 4.10; the fallback path is the one that runs, and it reported `identical()` on every component except `trial_success`.
**Figure 3b gate, revised `[Rev 2026-09-18]`.** The record's original criterion (optimal $w_{\text{PFS}}$ within 0.02 of the reference at $N=10^5$) is not achievable, because the expected gain is flat near its maximum: run with the paper's own procedure (Appendix B, "Figure 3b argmax noise"), the argmax at $N=10^5$ deviates from the $N=5\times10^6$ reference by up to 0.13 across the 21 cells on three seeds, while the gain at the reference optimum is within $2.2\times10^{-4}$ of the grid maximum. The gate is therefore stated on the gain, as a `skip_on_cran` block in `test-optimisation_gsd.R` with the paper's design inputs (PFS matured at look 1, OS at information fractions $(0.7, 1)$ with `sfLDOF`, $\rho=0.5$, nominal powers 0.98 and 0.93, $N=10^5$, one fixed seed): (1) for each of the 21 $(r,\delta)$ cells of `tests/testthat/data/gsd_example5_reference.rds`, the package's expected gain at the reference $w^*$ (nearest point of a 0.005 grid over $w_{\text{PFS}}$) is within $5\times10^{-4}$ of the package's own maximum over that grid, where the gains are formed in R from the `time_distribution` returned by `calc_power_pvals_gsd()` at each grid point, so that only one kernel run per point and no compilation per cell is needed; (2) at three cells ($r=1,\delta=1$; $r=4,\delta=0.75$; $r=8,\delta=0.5$) the compiled gain `!!v_pfs * d(t1) + !!v_os * d(t2)` agrees with the R formula on the same `time_distribution` to $10^{-12}$, and `graph_optimise_gsd()` with the transition matrix fixed at full recycling returns a $w_{\text{PFS}}$ whose gain is within $5\times10^{-4}$ of the grid maximum; (3) the argmax being non-increasing in $r$ at each $\delta$ is reported, not asserted. The paper computed its reference with gsDesign's classical O'Brien–Fleming boundary (`sfu = "OF"`) and a `<=` comparison; the interim nominal boundary at $\alpha=0.025$ is 0.0082 under OF and 0.0074 under `sfLDOF`, and the measured gain gaps are the same under both, so the standard spending function is used. What this gate demonstrates, and what it does not (P4 review): it shows that the package reproduces the paper's *gain surface* near its flat maximum, not the paper's *optimal weights*. At $N=3\times10^4$ between 59 and 114 of the 201 grid points lie within the $5\times10^{-4}$ tolerance of the maximum, and the whole curve over $w_{\text{PFS}}\in[0.05,0.95]$ spans only 0.0024 at $r=8,\delta=0.5$; the optimiser cell there returned $w_{\text{PFS}}=0.571$ against a reference of 0.469 and passed. That is the flatness the gate was written for; a reader who wants the optimal weight itself needs the paper's $N=5\times10^6$, and even then the argmax is resolved only to the width of the plateau.
...
|---|---|
| `test-transform_pvalues_gsd.R` | Table 1 and Table 2 reproduction (Appendix A values); monotonicity abort; grid accuracy; short-circuit identity; matured copy-forward; `NA` before first look gives 1; `cummin` under `look_back`; spend-at-1 check; information fractions above 1 accepted; oracle agreement with `graphicalMCP` (absolute $10^{-5}$, $K=2$) |
...
`[Rev 2026-09-18]` One bug-fix entry is added for the guard of 4.10:
...
Otherwise nothing in the fixed-sample path changes. The pre-existing issues noticed during this design should become GitHub issues: row subsampling permutes the first `nsim` rows rather than sampling from all rows (`[Rev 2026-09-18]` already fixed on `main` in `e62dfcf`, "fixes #12"); unary minus in the gain parser; `[Rev 2026-09-17]` the fixed-sample gain parser's placeholder substitution gives `&&`/`||` a higher precedence than `+`, so an unquoted `r1 + r2 && r3` is regrouped to `r1 + (r2 && r3)` after R had parsed it as `(r1 + r2) && r3` (4.6, "Parsing and precedence"); and a numeric literal that R deparses in exponent form (`1e-05`) is emitted as the invalid C++ `1e-05.0`. `[Rev 2026-09-18]` The Sol review adds that string input to `trial_success()` bypasses the operator whitelist, so `"r1 == 1"` or `"sqrt(r1)"` compile; two ready-to-file issue drafts (precedence; unary operators) are at the end of `dev/review/trial_success_gsd/ts_gsd_vignette_findings.md`. None of these is fixed in this roadmap (section 10 item 13).

## 10. Open items

1. **Function names.** CLOSED `[Rev 2026-09-16]`: `transform_pvalues_gsd()`, `simulate_pvalues_gsd()`, `graph_shortcut_gsd()` per the user's phase list; `trial_success_gsd()` is assumed for P3. `simulate_pvalues_gsd()` takes `power_nominal` with `corr_matrix` describing the final-look statistics, exactly as `simulate_pvalues()` (the record's 4.7 signature). This last point was applied by the orchestrator, not confirmed by the user; switching to noncentrality parameters is a small change to P1 if wanted. `corr_matrix` defaults to `diag(m)` as in `simulate_pvalues()`.
2. **`grid_size` default.** CLOSED: 1024. Measured errors at $t=(0.7,1)$ match Appendix B ($6.7\times10^{-7}$, $1.5\times10^{-6}$); 512 would suffice.
3. **Record location.** CLOSED: the record and brief are at `dev/`.
...
6. **`K` on the gain object.** Whether `K` must be supplied when no discount table fixes it, or may be left unset until first use. The record allows unset; a stricter rule is a one-line change. `[Rev 2026-09-17]` Implemented as recorded: `K` is `NULL` when neither a table nor the argument fixes it. P4 must check `K` against the `multigrain_pvals_gsd` object whenever it is set, and must never feed the compiled function a time value above `K` when tables exist: the table lookup `name_tab[t(i, idx)]` is an unchecked C array index, so a time above `K` (or an `NA`, which is `INT_MIN` in C) reads out of bounds. The kernel of 4.3 emits only `0..K`, so the documented pipeline is safe; the check belongs in `calc_power_pvals_gsd()` and `create_obj_func_gsd()`, next to the `anyNA()` assertion of item 11.
7. **Reference values for the Figure 3b gate.** CLOSED `[Rev 2026-09-18]`. The paper's code archive is at `origin/sample-size-pub:data-raw/Spiers_gain_function_code_for_outputs.zip`; its `Ex5-optimise.R` (with `src/quick_pfs_os.cpp`) and `results/Ex5-results.rds` are copied to `dev/review/fig3b/`, and the 21-row `w_star` table (with the design inputs) to `tests/testthat/data/gsd_example5_reference.rds`. The reference was computed at $N=5\times10^6$ on a 0.0005 grid with gsDesign's classical O'Brien–Fleming boundary. The argmax criterion had to be replaced by a gain criterion; section 6 P4 states the revised gate and Appendix B ("Figure 3b argmax noise") the measurements behind it.
...
9. **P0 review follow-ups** `[Rev 2026-09-16]`, none violating a LOCKED decision: (a) `alpha` validation is inclusive at 0 and 1 (`check_number_decimal(min = 0, max = 1)`), so both pass and fail inside `gsDesign`; should be exclusive. (b) A spending function returning `NA` reports a misleading length message. (c) An unnamed list of spending functions prints deparsed code as the `summary()` label; a named list prints cleanly. (d) The well-ordering check tolerates decreases below `sqrt(.Machine$double.eps) * max(column)` (about $3.7\times10^{-10}$ absolute) where 4.1 step 3 says non-decreasing; no realistic spending function escapes, but the record should say whether a tolerance is wanted and, if so, make it relative. All one-line changes; to be batched before P5.
10. **Duplicated interior information fractions** `[Rev 2026-09-16]`. 4.1 requires non-decreasing fractions, so `c(0.5, 0.5, 1)` is accepted by both the simulator (singular covariance, columns equal only to machine precision) and the transform's validation, and then the transform aborts with "fewer than two distinct positive boundaries" because the spend increment is zero. Decide whether to require strictly increasing fractions below 1 at input (recommended) or document the abort.
11. **`NA` in the kernel input** `[Rev 2026-09-16]`. The kernel treats `NA`/`NaN` as "never reject" silently (`NA < x` is false in C++) and no R caller validates the matrix yet. The transform never emits `NA`, so the pipeline is safe; `create_obj_func_gsd()` (P4) should assert `!anyNA()` once on the reshaped matrix. Also: kernel index arithmetic is `int`, so $N\cdot m\cdot K<2^{31}$. `[Rev 2026-09-18]` The assertion lives in `.gsd_kernel_matrix()` (`R/check_gsd.R`), which both consumers call once before the kernel, so the closure itself carries no check.
12. **Calendar-time metadata** `[Rev 2026-09-18]` (Sol item 7). The manuscript's $\tau_i$ is a calendar time and its discount function is a function of calendar time; the package's `t<i>` is a look index and the discount table is positional. That is correct and sufficient for the expected gain, but the analysis schedule is not auditable from the objects, a discount table cannot be checked against the schedule it was elicited for, and no calendar-scale report is possible. The recommended design is an optional `analysis_times` argument (length $K$, finite, strictly increasing, with a unit label) on `simulate_pvalues_gsd()` and `transform_pvalues_gsd()`, stored on the `multigrain_pvals_gsd` object and used by `calc_power_pvals_gsd()` to add `mean_decision_time` (mapping each non-zero look index to its time before averaging) and calendar labels on `time_distribution`; the kernel and the compiled gain stay on the index scale, and `trial_success_gsd()` does not take times (a schedule tied to one gain object could be paired with p-values from another). Deferred by user decision on 2026-09-18; P4 reports index-scale quantities only (4.4, 4.7).
13. **Fixed-sample parser findings** `[Rev 2026-09-18]` (Sol items 2 to 4): `trial_success()` regroups `&&`/`||` after R has parsed the expression (4.6), fails on unary `+`/`-`, and validates string input less strictly than expression input. All three are confirmed bugs in the fixed-sample path and are outside this roadmap (4.10); the issue drafts are in the findings file (section 9). The vignette and roxygen written on 2026-09-18 should describe them as known limitations, not conventions (P5).
...
Script run on 2026-09-15 with R 4.6.1 and gsDesign 3.11.0; re-run 2026-09-16 on the implementation machine with identical output apart from the two timings (`dev/gsd_check2_equivalence.R`). The `direct()` function is the Maurer and Bretz procedure with boundaries computed at the current weight on the fly (memoised); `transform()` and `kern()` are the design of sections 4.1 and 4.3 written in R. Three hypotheses, three looks: $H_1$ with LDOF spending at $(1/3, 2/3, 1)$, $H_2$ with HSD($-2$) spending maturing at look 2, $H_3$ mature at look 1. The four `look_back` configurations mix semantics across hypotheses. The clamp-safety block reproduces the red-team case of section 4.1. Two notes `[Rev 2026-09-16]`: the `P <- lapply(...)` block simulates each hypothesis independently (no cross-hypothesis $\rho$), which is sufficient for the equivalence check but is not the joint model of 4.7, so it must not be used as a reference for the simulator; and `matrix(p[, kk], N, K - kk)` warns "non-empty data for zero-extent matrix" when `kk == K` (harmless; RNG consumption is unaffected).
...
## Figure 3b argmax noise `[Rev 2026-09-18]`

Script `dev/review/fig3b/probe.R`, run on 2026-09-18 with R 4.6.1 and gsDesign 3.11.0; full output in `dev/review/fig3b/probe_output.txt`. It reproduces the paper's Example 5 procedure exactly (the archive's `quick_pfs_os.cpp` two-hypothesis kernel with boundaries from `gsDesign(sfu = "OF")`, the same `Egain()` construction, the same 21 $(r,\delta)$ cells) at $N=10^5$ and $N=10^6$ instead of $5\times10^6$, on a 0.005 grid instead of 0.0005, and compares the argmax and the gain at the reference $w^*$ with the archive's `Ex5-results.rds`. It also runs the same procedure with `sfLDOF` spending in place of the classical OF boundary.

| Setting | max $|\hat w - w^*|$ over 21 cells | mean | max gain gap at $w^*$ |
|---|---|---|---|
| OF, $N=10^5$, seed 1 | 0.115 | 0.040 | $2.0\times10^{-4}$ |
| OF, $N=10^5$, seed 2 | 0.118 | 0.026 | $1.7\times10^{-4}$ |
| OF, $N=10^5$, seed 3 | 0.130 | 0.042 | $2.2\times10^{-4}$ |
| LDOF, $N=10^5$, seed 1 | 0.113 | 0.039 | $2.2\times10^{-4}$ |
| LDOF, $N=10^6$, seed 1 | 0.083 | 0.018 | $5.9\times10^{-5}$ |

"Gain gap" is the expected gain at the grid maximum minus the expected gain at the reference $w^*$, on the same draws. The largest deviations sit at $r\ge2$ where the curves in the figure are steep in $r$ but the gain is flat in $w$: at $r=16,\delta=0.5$ the argmax at $N=10^5$ was 0.200 against a reference of 0.122 with a gain gap of $1.7\times10^{-4}$. Nominal boundaries at $(0.7,1)$, $\alpha=0.025$: OF 0.008197 and 0.022321; `sfLDOF` 0.007384 and 0.022754. The OF boundary's implied cumulative spend at full information is $\alpha$ to $5\times10^{-9}$ relative, so it could be supplied to the transform as a spending function (`function(a, t) cumsum(gsDesign::gsDesign(k = length(t), test.type = 1, alpha = a, sfu = "OF", timing = t)$upper$spend)`, about 10 ms per call), but nothing in the gate needs it.
