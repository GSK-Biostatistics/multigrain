...
No bug-fix entry: nothing in the fixed-sample path changes. The pre-existing issues noticed during this design should become GitHub issues: row subsampling permutes the first `nsim` rows rather than sampling from all rows; unary minus in the gain parser; `[Rev 2026-09-17]` the fixed-sample gain parser's placeholder substitution gives `&&`/`||` a higher precedence than `+`, so an unquoted `r1 + r2 && r3` is regrouped to `r1 + (r2 && r3)` after R had parsed it as `(r1 + r2) && r3` (4.6, "Parsing and precedence"); and a numeric literal that R deparses in exponent form (`1e-05`) is emitted as the invalid C++ `1e-05.0`.
