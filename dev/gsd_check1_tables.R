...
cat(
    "gamma=0.0125  :",
    format(nom_bounds(0.0125, t3), digits = 4),
    "  paper: 0.00002 0.0022 -\n"
)
cat(
    "gamma=0.01875 :",
    format(nom_bounds(0.01875, t3), digits = 4),
    "  paper k=2: 0.004\n"
)
cat(
    "gamma=0.00625 :",
    format(nom_bounds(0.00625, t3), digits = 4),
    "  paper k=2: 0.0008\n"
)
cat(
    "gamma=0.025   :",
    format(nom_bounds(0.025, t3), digits = 4),
    "  paper k=2: 0.006, k=3: 0.02313\n"
)
d <- gsDesign(k = 3, test.type = 1, alpha = 0.0125, timing = t3, sfu = sfLDOF)
cat(
    "cross-check gsDesign():",
    format(pnorm(d$upper$bound, lower.tail = FALSE), digits = 4),
    "\n"
)
...
rep_p <- function(p, k, t, sf = sfLDOF) {
    uniroot(
        function(g) nom_bounds(g, t, sf)[k] - p,
        c(1e-12, 0.999),
        tol = 1e-10
    )$root
}
...
r1 <- sapply(p1, rep_p, k = 1, t = t3)
r2 <- sapply(p2, rep_p, k = 2, t = t3)
...
repeat {
    ...
    for (l in 1:4) {
        for (k in 1:4) {
            Gn[l, k] <- if (l == k || rej[l] || rej[k]) {
                0
            } else {
                (G[l, k] + G[l, j] * G[j, k]) / (1 - G[l, j] * G[j, l])
            }
        }
    }
    w <- wn
...
cat(
    ...
    " order:",
    order,
    "  paper: H1, H2, H3 rejected; H4 retained\n"
)
